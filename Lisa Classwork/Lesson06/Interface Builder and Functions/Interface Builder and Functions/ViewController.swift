...
class ViewController: UIViewController {
    ...
    */
    ...
    @IBAction func helloButton(sender: AnyObject) {
        ...
        //or labelField.text = "Hello \(textFieldContent)"
    }
    ...
    //        labelField.text = "Hello \(textFieldContent)"
    
    // 1. get value of input field and convert to int
    let textFieldInteger = textField.text.toInt()!
    print("\(textFieldInteger)")
    
    // 2. send 1. to fib and store in let
    let fibNum = fib(fibPlace: textFieldInteger)
    print("\(fibNum)")
    
    // 3. set label value and convert 2. to string
    labelField.text = "\(fibNum)"
    
    //        let fibNum = fib(fibPlace: <#Int#>)*/
